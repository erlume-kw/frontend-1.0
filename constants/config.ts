...
// When deploying, either set the EXPO_PUBLIC_* env vars or change the fallback
...
export const MYFATOORAH_SCRIPT_URL =
  process.env.EXPO_PUBLIC_MYFATOORAH_SCRIPT_URL ??
...
/**
 * Web only: if the app was opened via "localhost", jump to the canonical dev
 * host so auth cookies, the persisted cart, and MyFatoorah redirects all share
 * one origin. No-op in production (real domain) and on native.
 */
