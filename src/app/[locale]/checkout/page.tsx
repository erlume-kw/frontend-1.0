...
export default function CheckoutPage() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (status !== undefined && status >= 500) {
            ...
            setAuthLoadError(t('loadAccountError'));
          } else {
  ...
  const startPayment = useCallback(async (payload: Parameters<typeof createOrder>[0]) => {
    ...
    try {
      ...
      setPayError(e.message ?? t('couldNotStart'));
  ...
  const handleWidgetResult = async (result: MFWidgetResult) => {
    ...
    if (!result.isSuccess && !result.paymentData) {
      setPayError(result.errorMessage || t('paymentIncomplete'));
      setPhase('failed');
      return;
    }
    ...
    try {
      ...
      if (verdict.success) {
        ...
        setPayError(verdict.message || t('notSuccessful'));
        setPhase('failed');
      }
    } catch (e: any) {
      setPayError(e.message ?? t('verificationFailed'));
      setPhase('failed');
    }
  };
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      setPayError(e.message ?? t('couldNotRestart'));
      setPhase('failed');
    }
  };
  ...
  const validateAddressFields = (newErrors: Record<string, string>) => {
    if (!governorate.trim()) newErrors.governorate = t('errGovernorate');
    if (!area.trim()) newErrors.area = t('errArea');
    if (!block.trim()) newErrors.block = t('errBlock');
    if (!street.trim()) newErrors.street = t('errStreet');
    if (!houseNumber.trim()) newErrors.houseNumber = t('errHouse');
    return newErrors;
  };

  const validateGuestForm = () => {
    const newErrors: Record<string, string> = {};
    if (!guestEmail.trim()) newErrors.guestEmail = t('errEmailRequired');
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestEmail)) newErrors.guestEmail = t('errEmailInvalid');
    if (!guestPhone.trim()) newErrors.guestPhone = t('errPhone');
  ...
  const handleVerifyEmail = async () => {
    if (!guestEmail.trim()) {
      setErrors(prev => ({ ...prev, guestEmail: t('errEmailRequired') }));
      return;
    }
    if (!isValidGuestEmail) {
      setErrors(prev => ({ ...prev, guestEmail: t('errEmailInvalid') }));
      return;
    }
    setSendingCode(true);
    try {
      ...
      setPayError(e?.message ?? t('sendCodeFailed'));
    } finally {
  ...
  const handleSaveNewAddress = async () => {
    ...
    if (!orderId) { setAddressError(t('orderNotReady')); return; }
    ...
    try {
      ...
      setAddressError(e.message ?? t('couldNotUpdateAddress'));
    } finally {
  ...
  const reprice = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (e?.status === 409 || e?.status === 404) { clearStoredOrder(); setOrderId(null); }
      setPayError(e.message ?? t('couldNotUpdateDiscount'));
      setPhase('failed');
    }
  }, [orderId]);
  ...
  const handleApplyDiscount = async () => {
    ...
    try {
      ...
      setDiscountError(e.message ?? t('invalidDiscount'));
      setDiscountAmount(0);
  ...
  const AddressFields = () => (
    <div className="flex flex-col gap-[14px]">
      ...
      <div className="flex h-[52px] flex-row items-center justify-between bg-[#E8E8E8] px-[11px]">
        <span className="flex-1 font-dm text-[14px] text-black">{t('kuwait')}</span>
      ...
      <div className={isDesktop ? 'flex flex-row gap-[14px]' : 'flex flex-col gap-[14px]'}>
        <input className={`${inputClass} ${isDesktop ? 'flex-1' : ''}`} placeholder={t('firstName')} value={firstName} onChange={e => setFirstName(e.target.value)} />
        <input className={`${inputClass} ${isDesktop ? 'flex-1' : ''}`} placeholder={t('lastName')} value={lastName} onChange={e => setLastName(e.target.value)} />
      </div>
      ...
      <div className={isDesktop ? 'flex flex-row gap-[14px]' : 'flex flex-col gap-[14px]'}>
        ...
        <input
          ...
          value={avenue}
          onChange={e => setAvenue(toWesternDigits(e.target.value).replace(/[^0-9]/g, ''))}
          inputMode="numeric"
        />
      </div>
  ...
  const SignedInAddress = () => {
    const active = customAddress
      ? {
          ...
          line: t('addressLine', {
            street: customAddress.shippingAddress.street,
            block: customAddress.shippingAddress.block,
            city: placeLabel(customAddress.shippingAddress.city),
            governorate: placeLabel(customAddress.shippingAddress.governorate),
            house: customAddress.shippingAddress.house,
          }),
          label: t('oneOff'),
        }
      : {
          title: userData?.emailAddress ?? t('yourAccount'),
          line: userData?.address
            ? t('addressLine', {
                street: userData.address.street ?? '',
                block: userData.address.block ?? '',
                city: placeLabel(userData.address.city ?? ''),
                governorate: placeLabel(userData.address.governorate ?? ''),
                house: userData.address.house ?? '',
              })
            : t('noAddress'),
    ...
    return (
      <div className="mb-7 flex flex-col gap-[14px]">
        <FormSection title={t('deliveryAddress')} />
        ...
        {!editingAddress ? (
          <>
            <div className="flex flex-col gap-[6px] border border-border bg-white p-4">
              <div className="flex flex-row items-center justify-between">
                ...
                <span className="bg-[rgba(197,112,93,0.12)] px-2 py-[3px]">
                  <span className="font-dm font-medium text-[11px] tracking-[0.8px] text-secondary">{t('selected')}</span>
                </span>
              </div>
            ...
            >
              <span className="font-clash font-medium text-[13px] tracking-[1px] text-black">{t('differentAddress')}</span>
            </button>
            {customAddress && (
              <button className="flex items-center justify-center py-[6px]" onClick={handleUseProfileAddress} disabled={addressSaving}>
                <span className="font-dm text-[13px] text-secondary underline">
                  {addressSaving ? t('switching') : t('switchBack')}
                </span>
              </button>
            )}
          </>
          ...
          <>
            ...
            >
              <span className="font-clash font-medium text-[13px] tracking-[1px] text-white">
                {addressSaving ? t('saving') : t('deliverHere')}
              </span>
            </button>
            <button className="flex items-center justify-center py-[6px]" onClick={() => { setEditingAddress(false); setAddressError(''); }}>
              <span className="font-dm text-[13px] text-secondary underline">{t('keepSelected')}</span>
            </button>
          </>
        )}
      </div>
  ...
  const CheckoutForm = () => (
    <div className={isDesktop ? 'max-w-[641px] flex-1 py-[38px]' : 'p-[21px]'}>
      ...
      >
        <span className="font-clash font-medium text-[13px] uppercase tracking-[1px] text-muted">{t('back')}</span>
      </button>
      ...
      {isLoggedIn ? (
        <>
          <div className="mb-7 flex flex-col gap-[14px]">
            ...
            <span className="font-dm text-[14px] text-muted">
              {t('signedInAs', { who: userData?.emailAddress ?? t('yourAccountLower') })}
            </span>
          </div>
          {SignedInAddress()}
        </>
        ...
        <>
          ...
          <div className="mb-7 flex flex-col gap-[14px]">
            <FormSection title={t('guestCheckout')} />
            <span className="font-dm text-[14px] text-muted">
              {t('haveAccount')}{' '}
              <button onClick={() => { if (requestLeave('/sign-in')) router.push('/sign-in'); }}>
                <span className="text-secondary underline">{t('signIn')}</span>
              </button>
            </span>
          </div>
          ...
          <div className="mb-7 flex flex-col gap-[14px]">
            ...
            <div>
              <div className="flex flex-row">
                ...
                />
                {guestEmailVerified ? (
                  <div className="flex h-[52px] items-center bg-lightGrey pe-[14px]">
                    <span className="font-dm text-[13px] font-medium text-[#2E7D4F]">{t('verified')}</span>
                  </div>
                ) : (
                  <button
                    type="button"
                  ...
                  >
                    <span className="font-clash font-medium text-[13px] uppercase tracking-[1.2px] text-white">
                      {sendingCode ? t('sending') : t('verify')}
                    </span>
                  </button>
                )}
              </div>
            ...
            <div>
              <input
                ...
                value={guestPhone}
                onChange={e => {
                  setGuestPhone(toWesternDigits(e.target.value));
          ...
          <div className="mb-7 flex flex-col gap-[14px]">
            ...
            <span className="mt-1 font-clash font-medium text-[16px] text-black">{t('deliveryMethods')}</span>
            <div className="overflow-hidden">
              <div className="flex flex-row items-center gap-3 bg-[#EFF5FF] px-4 py-[14px]">
                ...
                <div className="flex flex-col gap-[2px]">
                  <span className="font-dm text-[14px] text-black">{t('express')}</span>
                  <span className="font-dm text-[12px] text-muted">{t('withinKuwait')}</span>
                </div>
              </div>
            </div>

            <span className="mt-1 font-clash font-medium text-[16px] text-black">{t('addressTitle')}</span>
            {AddressFields()}
          </div>
        </>
      )}
    ...
    <div className="mt-1 flex flex-col gap-3">
      ...
      {(phase === 'idle' || phase === 'initiating' || phase === 'paying') && (
        <span className="font-dm text-[13px] leading-[21px] text-muted">
          {t.rich('agree', {
            terms: chunks => (
              <Link href="/terms" target="_blank" rel="noreferrer" className="text-secondary underline">
                {chunks}
              </Link>
            ),
            returns: chunks => (
              <Link href="/returns" target="_blank" rel="noreferrer" className="text-secondary underline">
                {chunks}
              </Link>
            ),
          })}
      ...
      {(phase === 'idle' || phase === 'initiating') && isLoggedIn && (
        <div className="flex flex-col items-center gap-2 py-4">
          <span className="font-dm text-[14px] text-muted">{t('preparingSecure')}</span>
        </div>
      )}
      ...
      {(phase === 'paying' || phase === 'verifying') && sessionId && (
        <>
          <span className="font-dm text-[14px] text-muted">{t('secure')}</span>
          <MyFatoorahEmbed sessionId={sessionId} onResult={handleWidgetResult} />
          {phase === 'verifying' && (
            <div className="flex flex-col items-center gap-2 py-4">
              <span className="font-dm text-[14px] text-muted">{t('verifyingPayment')}</span>
            </div>
          )}
        </>
      )}
  ...
  const OrderSummary = () => (
    <div className={isDesktop ? 'min-h-[600px] w-[582px] bg-lightGrey p-[38px]' : 'p-[21px]'}>
      <span className="mb-1 block font-clash font-semibold text-[18px] text-black">{t('summary')}</span>

      {cartItems.map(item => (
        <div key={item.id} className="mb-1 flex flex-row justify-between py-1">
          <span className="me-2 flex-1 truncate font-dm text-[13px] text-black">
            {item.brand} — {item.name}
          </span>
          <span className="font-dm font-semibold text-[13px] text-black">{money(item.price)}</span>
        </div>
      ))}
      ...
      <div className="mb-4 mt-3 flex flex-row gap-[14px]">
        ...
        <button className="flex h-[52px] items-center justify-center bg-lightGrey px-4" onClick={handleApplyDiscount}>
          <span className="font-dm font-medium text-[14px] text-black">{t('apply')}</span>
        </button>
      </div>
      {!!discountError && <div className="mb-2 font-dm text-[13px] text-error">{discountError}</div>}
      {discountAmount > 0 && (
        <div className="mb-2 font-dm text-[13px] text-olive">
          {t.rich('discountLine', {
            amount: money(discountAmount.toFixed(2)),
      ...
      {[
        { label: t('subtotal', { count: cartItems.length, n: num(cartItems.length) }), value: money(subtotal.toFixed(2)) },
      ...
      <div className="mb-4 flex flex-row justify-between py-3">
        <span className="font-dm font-semibold text-[16px] text-black">{t('total')}</span>
        <span className="font-dm font-semibold text-[16px] text-black">{money(total.toFixed(2))}</span>
      </div>
  ...
  return (
    ...
    >
      ...
      <ErrorModal
        ...
        title={t('verifyPromptTitle')}
        message={t('verifyPromptText')}
      ...
      <MaxWidthContainer className={isDesktop ? 'px-16' : ''}>
        {phase === 'success' ? (
          ...
          <div className="flex flex-col items-center py-20">
            <span className="font-dm text-[14px] text-muted">{t('loading')}</span>
          </div>
        ) : authLoadError ? (
          <div className="flex flex-col items-center gap-4 py-20">
            ...
            >
              <span className="font-clash font-medium text-[14px] uppercase tracking-[1.2px] text-white">{t('refresh')}</span>
            </button>
          </div>
        ) : isDesktop ? (
